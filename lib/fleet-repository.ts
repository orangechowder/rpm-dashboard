...
function mapUnit(row: Record<string, unknown>): CloudUnit {
  return { unit: String(row.unit), vin: String(row.vin), client: String(row.client), type: String(row.type), service: String(row.service), due: String(row.due), overdue: Boolean(row.overdue), usage: String(row.usage), currentMeter: row.current_meter == null ? null : Number(row.current_meter), lastPmMeter: row.last_pm_meter == null ? null : Number(row.last_pm_meter), pmInterval: Number(row.pm_interval ?? 25000), meterUnit: row.meter_unit === "HRS" ? "HRS" : "KM" };
}

function mapJob(row: Record<string, unknown>): CloudJob {
  return { id: String(row.id), unit: String(row.unit), client: String(row.client), tech: String(row.tech), priority: row.priority as CloudJob["priority"], status: row.status as CloudJob["status"], issue: String(row.issue), updated: String(row.updated), usage: String(row.usage), meterReading: row.meter_reading == null ? null : Number(row.meter_reading), notes: Array.isArray(row.notes) ? row.notes : [], lineItems: Array.isArray(row.line_items) ? row.line_items : [] };
}
...
export async function saveUnits(units: CloudUnit[]) {
  if (!supabase) return;
  const meterRows = units.map((unit) => ({ unit: unit.unit, vin: unit.vin, client: unit.client, type: unit.type, service: unit.service, due: unit.due, overdue: unit.overdue, usage: unit.usage, current_meter: unit.currentMeter ?? null, last_pm_meter: unit.lastPmMeter ?? null, pm_interval: unit.pmInterval ?? 25000, meter_unit: unit.meterUnit ?? "KM", updated_at: new Date().toISOString() }));
  const result = await supabase.from("fleet_units").upsert(meterRows, { onConflict: "unit" });
  ...
  const legacyRows = units.map((unit) => ({ unit: unit.unit, vin: unit.vin, client: unit.client, type: unit.type, service: unit.service, due: unit.due, overdue: unit.overdue, usage: unit.usage, updated_at: new Date().toISOString() }));
  const legacyResult = await supabase.from("fleet_units").upsert(legacyRows, { onConflict: "unit" });
  if (legacyResult.error) throw legacyResult.error;
}

export async function saveJobs(jobs: CloudJob[]) {
  if (!supabase) return;
  const meterRows = jobs.map((job) => ({
    id: job.id,
    unit: job.unit,
    client: job.client,
    tech: job.tech,
  ...
  const result = await supabase.from("work_orders").upsert(meterRows, { onConflict: "id" });
  if (!result.error) return;
  ...
  const legacyRows = jobs.map((job) => ({
    ...
    notes: job.notes ?? [], line_items: job.lineItems ?? [], updated_at: new Date().toISOString(),
  }));
  const legacyResult = await supabase.from("work_orders").upsert(legacyRows, { onConflict: "id" });
  if (legacyResult.error) throw legacyResult.error;
}
